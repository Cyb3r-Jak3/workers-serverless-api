...
describe('Gravatar Endpoints', () => {
    it('Missing GET Gravatar', async () => {
        const resp = await exports.default.fetch('http://localhost/misc/gravatar/')
        expect(resp.status).toBe(404)
    })
    it('GET Gravatar', async () => {
        const resp = await exports.default.fetch('http://localhost/misc/gravatar/git@cyberjake.xyz')
