...
describe('PyPy Endpoints', () => {
    ...
    it('PyPy Version', async () => {
        const request = new Request('https://localhost/pypy/checksums/pypy3.9-v7.3.11')
        const ctx = createExecutionContext();
        const resp = await worker.fetch(request, env, ctx)
        await waitOnExecutionContext(ctx)
        expect(resp.status).toBe(200)
    ...
    it('PyPy 3.10 Version', async () => {
        const request = new Request('https://localhost/pypy/checksums/pypy3.10-v7.3.12')
        const ctx = createExecutionContext();
        const resp = await worker.fetch(request, env, ctx)
        await waitOnExecutionContext(ctx)
        expect(resp.status).toBe(200)
    ...
    it('PyPy File', async () => {
        const request = new Request('https://localhost/pypy/checksums/pypy3.9-v7.3.11-src.tar.bz2')
        const ctx = createExecutionContext();
        const resp = await worker.fetch(request, env, ctx)
        await waitOnExecutionContext(ctx)
        expect(resp.status).toBe(200)
